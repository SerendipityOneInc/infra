...
type Config struct {
	...
	// validate an upload URL another one signed (Traefik load-balances
	// /upload with no session affinity). Falls back to a per-process random
	// key when unset, which is only safe with a single allocation.
